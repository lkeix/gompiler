...
func expect(token *Token, op byte) *Token {
	if token.kind != Reserved || token.token != string(op) {
...
func newToken(kind TokenKind, current *Token, token string, length int) *Token {
	next := &Token{
		kind:   kind,
		token:  token,
		length: length,
...
func tokenize(code string) *Token {
	...
	for i := 0; i < len(code); i++ {
		...
		if code[i] == '+' ||
			...
			code[i] == '(' ||
			code[i] == ')' {
			current = newToken(Reserved, current, string(code[i]), 1)
			continue
		}
		...
		if code[i:i+1] == "==" ||
			...
			code[i:i+1] == ">" ||
			code[i:i+1] == "!=" {
			current = newToken(Reserved, current, string(code[i:i+1]), 2)
			continue
		}
		...
		if unicode.IsDigit(rune(code[i])) {
			...
			current = newToken(Number, current, code[i:j], 1)
			current.val = val
			i = j - 1
			continue
		}
	...
	current = newToken(EOF, current, code[:], 0)
	return head.next
}
...
func (o *parser) mul() *Node {
	...
	for {
		if o.token, ok = consume(o.token, '*'); ok {
			node = newBinary(NMul, node, o.unary())
		}
		if o.token, ok = consume(o.token, '/'); ok {
			node = newBinary(NDiv, node, o.unary())
		}
		return node
	}
}
...
func (o *parser) unary() *Node {
	ok := false
	if o.token, ok = consume(o.token, '+'); ok {
		return o.primary()
	}
	if o.token, ok = consume(o.token, '-'); ok {
		return newBinary(NSub, newNumber(0), o.primary())
	}
	return o.primary()
}
